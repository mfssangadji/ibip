    ...
    <main role="main" class="d" style="width: 100%">
      ...
      <div class="box-body">
        <table id="datapost" class="table table-bordered table-striped">
          <thead>
          <tr>
            <th colspan="100">{{@$perizinan->last()->kepel->kepel}}</th>
          </tr>
          <tr>
            <th>ID</th>
          ...
          <tbody>
            @foreach($perizinan as $p)
              <tr>
                ...
                <td>{{$jenis_perizinan[$p->jenis_perizinan]}}</td>
                <td>{{$status_perizinan[$p->status]}}</td>
                <td>{{$p->created_at->format('d F Y')}}</td>
                <td>
                  @if(@$p->id == @Auth::user()->ibip->perizinan_id)
                    -
                  @else
                    <a href="{{url('perizinan/'.$p->id.'/pengajuan-ijin')}}">Ijin {{$jenis_perizinan[$p->jenis_perizinan]}}</a>
                  @endif
                </td>
              </tr>
            @endforeach
